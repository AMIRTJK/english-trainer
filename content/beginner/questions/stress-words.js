import { makeQuestions, stressOption } from './build';
const rows = [
...
const drafts = rows.map(([word, syl, correct, picks, ipa]) => {
    const options = picks.map((p) => stressOption(syl, p));
    const answer = picks.indexOf(correct);
    return {
...
export const stressWords = makeQuestions({
