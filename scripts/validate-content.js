...
function checkStructure(q) {
    if (q.options.length !== 3)
        err(q, `expected 3 options, got ${q.options.length}`);
    if (q.answer < 0 || q.answer > 2)
        err(q, `answer index ${q.answer} out of range`);
    const trimmed = q.options.map((o) => o.trim().toLowerCase());
    if (new Set(trimmed).size !== 3)
        err(q, `duplicate options: ${q.options.join(' | ')}`);
    if (q.options.some((o) => o.trim() === ''))
        err(q, 'empty option');
    if (!q.explanation.trim())
        err(q, 'missing explanation');
    if (!q.source.ref)
        err(q, 'missing source reference');
    if (!q.constructId)
        err(q, 'missing constructId');
...
function checkSound(q) {
    if (q.type !== 'different-sound')
        return;
    if (!q.sound) {
...
function checkStress(q) {
    ...
    if (marked.length !== 3)
        err(q, 'every option must mark exactly one stressed syllable');
    ...
    for (const level of LEVELS) {
        ...
        for (const q of index.content.questions) {
            ...
            if (!topicIds.has(q.topicId))
                err(q, `unknown topicId ${q.topicId}`);
            if (q.levelId !== level.id)
                err(q, `levelId ${q.levelId} does not match ${level.id}`);
            if (q.status !== 'verified')
                err(q, `status ${q.status} must not be in the active pool`);
            checkStructure(q);
            checkLexicon(q);
            checkSound(q);
            checkStress(q);
            ...
            if (prev)
                errors.push(`identical question in ${prev} and ${q.id}`);
            else
            ...
            if (prevOrder && prevOrder !== q.id) {
                ...
                seenOptionSets.set(orderKey, q.id);
        }
        const emptyTopics = index.content.topics.filter((t) => !(index.byTopic.get(t.id)?.length));
        for (const t of emptyTopics)
            warnings.push(`topic "${t.title}" (${t.id}) has no questions`);
        perLevel.push(`${level.name}: ${index.content.questions.length} verified questions, ` +
            `${index.content.topics.length} topics, ${index.byConstruct.size} constructs`);
    }
    console.log('\nContent validation');
    console.log('==================');
    for (const line of perLevel)
        console.log('  ' + line);
