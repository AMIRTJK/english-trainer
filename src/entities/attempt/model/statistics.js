...
function emptyStat(topicId, categoryId) {
    return {
        topicId,
        categoryId,
        seen: 0,
 ...
 */
export function applyAttempt(progress, attempt) {
    for (const answer of attempt.answers) {
        ...
        if (answer.correct)
            ...
            stat.lastErrorAt = attempt.finishedAt;
...
export function confidenceOf(stat) {
    if (stat.seen === 0)
        return 'no-data';
    if (stat.seen < MIN_OBSERVATIONS || stat.seenQuestionIds.length < MIN_UNIQUE_QUESTIONS) {
        return 'low';
    }
    if (stat.seen < MIN_OBSERVATIONS * 3)
        return 'medium';
    return 'high';
}
export function summarise(stat) {
...
export function categoryBreakdown(stats) {
    return foldCategories(Object.values(stats));
}
/** Per-attempt score by category, used on the results screen. */
export function attemptCategoryBreakdown(attempt) {
    return foldCategories(attempt.answers.map((a) => ({ categoryId: a.categoryId, seen: 1, correct: a.correct ? 1 : 0 })));
}
