...
export interface LevelData {
  ...
  levelName: string;
  hasContent: boolean;
  index: ReturnType<typeof getLevelIndex>;
  progress: LevelProgress;
  /** One row per topic that has content, whether or not it has been practised. */
  summaries: TopicSummary[];
  byTopicId: Map<string, TopicSummary>;
}

export function useLevelData(): LevelData {
  const { user } = useApp();
...
export function weakAreas(summaries: TopicSummary[], limit = 5): TopicSummary[] {
  return summaries
    .filter((s) => s.seen > 0 && s.status !== 'strong')
    .sort((a, b) => {
      // Confirmed weak topics rank above ones that are merely unproven.
      const rank = (s: TopicSummary): number => (s.status === 'weak' ? 0 : s.status === 'review' ? 1 : 2);
