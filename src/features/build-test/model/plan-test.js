...
import { splitByMix } from './presets';
/**
 ...
 */
export function planTest(options, progress) {
    ...
    if (!usesMix)
        return generateTest(base, progress);
    ...
    quotas.forEach(([categoryId, wanted], i) => {
        if (wanted === 0)
            return;
        const plan = generateTest({ ...base, count: wanted, categoryIds: [categoryId], seed: seed + i * 7919 }, progress);
    ...
    if (shortfall > 0) {
        warnings.push(`The verified question bank can supply ${items.length} unique questions for this test, ` +
