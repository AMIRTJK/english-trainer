 ...
 */
export const OFFICIAL_MIX: CategoryMix = {
  grammar: 0.4,
  vocabulary: 0.4,
  pronunciation: 0.1,
  'word-stress': 0.1,
...
export const PRACTICE_MIX: CategoryMix = {
  grammar: 0.25,
  vocabulary: 0.25,
  pronunciation: 0.25,
  'word-stress': 0.25,
...
export function findPreset(kind: TestKind): TestPreset | undefined {
  return PRESETS.find((p) => p.kind === kind);
}

/** Split a total into whole numbers per category, preserving the total. */
export function splitByMix(total: number, mix: CategoryMix): Array<[CategoryId, number]> {
  const entries = Object.entries(mix) as Array<[CategoryId, number]>;
  const raw = entries.map(([id, share]) => [id, total * share] as const);
  const out = raw.map(([id, value]) => [id, Math.floor(value)] as [CategoryId, number]);
