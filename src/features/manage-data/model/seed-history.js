import { IMPORTED_ITEMS, IMPORTED_TEST_META, MISTAKE_DETAIL, } from '@content/beginner/imported-quick-test';
 ...
 * the note says so.
 */
export function buildImportedAttempt(levelId) {
    const index = getLevelIndex(levelId);
    if (!index)
        return null;
    const answers = IMPORTED_ITEMS.map((item) => {
    ...
    return {
        ...
        note: `Imported from the paper ${IMPORTED_TEST_META.paper}. ` +
...
export function seedImportedHistory(progress) {
    if (progress.attempts.some((a) => a.id === IMPORTED_ATTEMPT_ID))
        return false;
